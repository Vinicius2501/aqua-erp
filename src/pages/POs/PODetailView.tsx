...
import { PaymentTimelineDialog } from "@/components/po/PaymentTimelineDialog";
import {
  purchaseOrders,
  users,
  costCenters,
...
const PODetailView = () => {
  ...
  const fromApprovals = (location.state as { from?: string })?.from === 'approvals';
  ...
  useEffect(() => {
    const loadPO = async () => {
      ...
      try {
        ...
        // Buscar histórico de pagamentos das parcelas
        const poInstallments = installmentPaymentHistory.filter(
          (inst) => inst.purchaseOrderId === id
        );
        setInstallments(poInstallments);
  ...
  return (
    <AppLayout>
      <div className="space-y-6 pb-3">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex items-center gap-4">
              <Button
                ...
                className="shrink-0"
                title={fromApprovals ? t('approvals.title') : t('poList.title')}
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-2.5">
            ...
            <CollapsibleCard
              ...
              storageKey={
                id
                  ? `po-${id}-accounting-allocation`
                  : "po-accounting-allocation"
              }
            >
            ...
            <CollapsibleCard
              ...
              defaultOpen={false}
              storageKey={id ? `po-${id}-payment` : "po-payment"}
            >
            ...
            <CollapsibleCard
              ...
              defaultOpen={true}
              storageKey={id ? `po-${id}-installments` : "po-installments"}
            >
            ...
            <CollapsibleCard
              ...
              defaultOpen={false}
              storageKey={id ? `po-${id}-general-info` : "po-general-info"}
            >
              <div className="grid grid-cols-1 sm:grid-cols-2">
                ...
                <InfoItem
                  ...
                  value={
                    po.beneficiary?.name === "FUNDO_GP" ? "Fundo GP" : "PortCo"
                  }
                />
                <InfoItem
                ...
                <InfoItem
                  ...
                  value={po.isIcApproved ? t("poDetail.yes") : t("poDetail.no")}
                  badge={
                    po.isIcApproved ? (
            ...
            <CollapsibleCard
              ...
              defaultOpen={false}
              storageKey={id ? `po-${id}-supplier` : "po-supplier"}
            >
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <InfoItem
          ...
          <div className="lg:sticky lg:top-20 lg:self-start lg:max-h-[calc(100vh-6rem)] lg:overflow-y-auto space-y-3">
            {/* Card Unificado: Histórico e Detalhes */}
            <Card className="pt-0 pb-0.5 shadow-sm">
            ...
            {/* Botão para ver timeline de pagamentos */}
            {po.installmentCount && po.installmentCount > 1 && (
              <Button
                size="sm"
                className="w-full bg-card text-foreground hover:bg-card/80 border border-input cursor-pointer"
                onClick={() => setShowPaymentTimeline(true)}
              >
                <History className="h-4 w-4 mr-2" />
                Ver Timeline de Pagamentos
              </Button>
            )}
          </div>
        </div>
        ...
        <div className="flex justify-end gap-3">
          <Button variant="secondary" onClick={handleGoBack}>
            {fromApprovals ? t('approvals.title') : t("common.back")}
          </Button>
          <Button
            onClick={() =>
      ...
      {/* Dialog de Timeline de Pagamentos */}
      <PaymentTimelineDialog
        open={showPaymentTimeline}
        onOpenChange={setShowPaymentTimeline}
